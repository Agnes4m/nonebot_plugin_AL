...
async def save_img_ship(tag: Optional[str] = None, temp: bool = True):
    ...
    if not tag:
        with open(
            SAVE_PATH.joinpath("ship_html", "ship_info.html"), "r", encoding="utf-8"
        ) as f:
            html = f.read()
        ...
        with open(
            SAVE_PATH.joinpath("ship_html", f"ship_{tag}.html"), "r", encoding="utf-8"
        ) as f:
            html = f.read()
        pic = await html_to_pic(html=html)
        with open(
            SAVE_PATH.joinpath("ship_html", "images", f"ship_{tag}{tep}.png"), "wb"
        ) as f:
            f.write(pic)
...
def print_img_ship_retrofit():
    ...
    print("开始")
    imgkit.from_file(
        SAVE_PATH.joinpath("ship_html", "ship_retrofit.html"),
...
def print_img_skin():
    ...
    options = {"encoding": "UTF-8", "enable-local-file-access": None}
    imgkit.from_file(
        SAVE_PATH.joinpath("ship_html", "ship_skin.html"),
...
def img_process_ship_weapon():
    ...
    img = cv2.imread(
        str(SAVE_PATH.joinpath("ship_html", "images", "ship_weapon_temp.png"))
    )
    image = img.shape
    cropped = img[0 : image[0], 0:620]  # 裁剪坐标为[y0:y1, x0:x1]
    cv2.imwrite(
        str(SAVE_PATH.joinpath("ship_html", "images", "ship_weapon.png")), cropped
    )
...
async def get_local_version():
    async with aiofiles.open(
        SAVE_PATH.joinpath("azurapi_data", "version-info.json"), "r", encoding="utf-8"
    ) as load_f:
