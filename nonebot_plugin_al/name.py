...
import aiofiles
import difflib
from .config import azur_config
...
SAVE_PATH = DATA_PATH
MAIN_URL = (
    f"{azur_config.proxy}AzurAPI"  ##https://raw.fastgit.org可以根据需要配置不同代理，结合网络情况自行修改
)
...
async def UpdateName():
    ...
    async with aiofiles.open(
        DATA_PATH_STR + "/azurapi_data/ships.json", "r", encoding="utf-8"
    ) as fp:
        load_dict = str(await fp.read())
    ...
    # print(ships)
    async with aiofiles.open(
        DATA_PATH_STR + "/azurapi_data/names.json", "w", encoding="utf-8"
    ) as fp:
