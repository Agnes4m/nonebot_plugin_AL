...
async def _(matcher: Matcher, args: Message = CommandArg()):
    ...
    if word in tags:
        ...
        # 舰船搜索
        with open(
            (Path(__file__).parent.joinpath("data/ship.json")),
            mode="r",
            encoding="utf-8",
        ) as f:
            ships = json.load(f)
        ...
        with open(
            (Path(__file__).parent.joinpath("data/eq.json")), mode="r", encoding="utf-8"
        ) as f:
            eq = json.load(f)
...
# 以下为移植的内容
on_command("blhx", block=True, priority=50, handlers=[blhx.send_ship_skin_or_info])

on_command("blhx 过场", block=True, priority=10, handlers=[blhx.send_random_gallery])

on_command("blhx 帮助", block=True, priority=10, handlers=[blhx.send_blhx_help])
on_command("blhx 强度榜", block=True, priority=10, handlers=[blhx.send_pve_recommendation])
on_command(
    "blhx 强制更新",
    block=True,
    priority=10,
...
on_command("blhx 最新活动", block=True, priority=10, handlers=[blhx.get_recently_event])
on_command(
    "blhx 备注", block=True, priority=10, permission=ADMIN, handlers=[blhx.set_nickname]
)
on_command(
    "blhx 移除备注",
    block=True,
...
on_command("blhx 皮肤", block=True, priority=10, handlers=[blhx.quick_search_skin])
on_command("blhx 大建", block=True, priority=10, handlers=[blhx.building])
