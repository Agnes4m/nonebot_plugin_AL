...
class BLHX_BASE:
    ...
    async def send_ship_skin_or_info(
        matcher: Matcher, event: Event, bot: Bot, arg: Message = CommandArg()
    ):
        ...
        try:
            ...
            if len(args) == 2:
                ...
                if flag == 0:
                    ...
                    msg = MessageSegment.image(
                        "file:///" + str(SAVE_PATH.joinpath("images/ship_skin.png"))
                    )
                    await bot.send(event=event, message=msg, at_sender=True)
                if flag == 1:
                    msg = "她只有原皮！"
            ...
            if len(args) == 1:
                ...
                index = await format_data_into_html(
                    await get_ship_data_by_id(ship_nickname_data["id"])
                )
                ...
                if index == 0:
                    msg = (
                        ...
                        + MessageSegment.image(
                            "file:///" + str(SAVE_PATH.joinpath("images/ship_info.png"))
                        )
                        + "\n推荐出装\n"
                        + MessageSegment.image(
                            "file:///"
                            + str(SAVE_PATH.joinpath("images/ship_weapon.png"))
                        )
                        + "\n此船备注昵称有：\n"
                        + nickname_list
                    )
                    ...
                    msg = (
                        ...
                        + MessageSegment.image(
                            "file:///"
                            + str(SAVE_PATH.joinpath("images/ship_retrofit.png"))
                        )
                        + "\n推荐出装\n"
                        + MessageSegment.image(
                            "file:///"
            ...
            if len(args) == 0:
                await bot.send(
                    event=event, message="请在命令之后提供精确舰船名称和皮肤昵称哦~", at_sender=True
                )
        except Exception as e:
            traceback.print_exc()
            await bot.send(event=event, message=f"查询出错,{e}", at_sender=True)

    async def send_random_gallery(matcher: Matcher, event: Event, bot: Bot):
        SAVE_PATH = (
            Path()
            .cwd()
            .joinpath("data/al/ship_html/images/gallery/", get_random_gallery())
        )
        msg = MessageSegment.image("file:///" + str(SAVE_PATH))
        await bot.send(event=event, message=msg, at_sender=True)

    async def send_blhx_help(matcher: Matcher, event: Event, bot: Bot):
        msg = """
    ...
    async def send_pve_recommendation(matcher: Matcher, event: Event, bot: Bot):
        ...
        if len(div_list) != 0:
            for i in range(0, len(div_list)):
                msg += Message(
                    str(div_list[i].find("img")["alt"])
                    + MessageSegment.image(file=str(div_list[i].find("img")["src"]))
                    + "\n"
                )
            msg_list.append(msg)
            forward_msg = render_forward_msg(msg_list, bot=bot)
    ...
    async def force_update(
        ...
        try:
            await bot.send(
                event=event,
                message="正在更新，首先请确保网络能访问github的文件中心，否则容易出现翻车风险！",
                at_sender=True,
            )
            ...
            await bot.send(
                event=event,
                message="强制更新完成.强制更新内容仅在离线模式下有效，当前版本V" + version,
                at_sender=True,
            )
        except:
            # 出问题赶紧回滚
    ...
    async def get_recently_event(matcher: Matcher, event: Event, bot: Bot):
    ...
    async def building(
        ...
        try:
            ...
            if len(args) == 1:
                ...
                await bot.send(
                    event=event, message="指令错误 发送blhx大建 轻型/重型/特型 进行大建模拟", at_sender=True
                )
                return
        except:
            msg = "处理出错，请看日志"
