...
def WriteText(
    image, font, text="text", pos=(0, 0), color=(255, 255, 255, 255), anchor="lt"
):
    # 这个函数向图片上写入字符，不提供给外部调用
    rgba_image = image.convert("RGBA")
    text_overlay = Image.new(
        "RGBA", rgba_image.size, (255, 255, 255, 0)
    )  # 创建一张空白的透明图像，大小和之前的图像一致，否则无法合成
...
class GachaImage:
    ...
    async def MakeSinglePic(self, info):
        ...
        im = Image.new(
            "RGBA",
            (GACHA_SIZE[info["rarity"]][0], GACHA_SIZE[info["rarity"]][1] + 5),
            (0, 0, 0, 0),
        )  # 透明底板
        ...
        DrawImage(
            ...
            im_bg,
            (
                int((im.size[0] - im_bg.size[0]) / 2),
                int((im.size[1] - im_bg.size[1]) / 2),
            ),
        )
        # 绘制角色
        try:
        ...
        DrawImage(
            ...
            im_ch,
            (
                int((im.size[0] - im_ch.size[0]) / 2),
                int((im.size[1] - im_ch.size[1]) / 2),
            ),
        )
        # 绘制两个黑条
        ...
        DrawImage(
            im,
            im_bl,
            (int((im.size[0] - im_bl.size[0]) / 2), int(im.size[1] / 2) - 148),
            0.4,
        )
        DrawImage(
            im,
            im_bl,
            (int((im.size[0] - im_bl.size[0]) / 2), int(im.size[1] / 2) + 91),
            0.4,
        )
        # 绘制类型
        ...
        DrawImage(
            im, im_tp, (int(im.size[0] / 2) - 107, int(im.size[1] / 2) - 144), 0.95
        )
        # 绘制等级
        text = Datatext(
            int(im.size[0] / 2) + 45,
        ...
        DrawImage(
            ...
            im_ed,
            (
                int((im.size[0] - im_ed.size[0]) / 2),
                int((im.size[1] - im_ed.size[1]) / 2),
            ),
        )
        # 绘制星星
        ...
        DrawImage(
            im, im_st, (int((im.size[0] - im_st.size[0]) / 2), int(im.size[1] / 2) + 124)
        )
    ...
    async def Make(self):
        ...
        for i in range(self.ship_num):
            ...
            DrawImage(
                ...
                im_si,
                (
                    GACHA_OFFSET[i][0] - int(im_si.size[0] / 2),
                    GACHA_OFFSET[i][1] - int(im_si.size[1] / 2),
                ),
            )
        # im.show()
